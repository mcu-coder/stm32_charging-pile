...
static void bh1750_i2c_init(void)
{
    ...
    // MX_I2C1_Init();
    if (i2c_CheckDevice(BH1750_ADDR_WRITE) == 0) {
        printf("bh1750 exist!\n");
    } else {
        printf("bh1750 is not exist!\n");
    }
}

static int bh1750_send_cmd(uint8_t cmd)
{
    //return HAL_I2C_Master_Transmit(&BH1750_I2C_HANDLE, BH1750_ADDR_WRITE, &cmd, 1, 20);
    return i2c_transmit(BH1750_ADDR_WRITE, &cmd, 1, 20);
}

static int bh1750_read_dat(uint8_t* dat)
{
    //return HAL_I2C_Master_Receive(&BH1750_I2C_HANDLE, BH1750_ADDR_READ, dat, 2, 20);
    return i2c_receive(BH1750_ADDR_WRITE, dat, 2, 20);
}
