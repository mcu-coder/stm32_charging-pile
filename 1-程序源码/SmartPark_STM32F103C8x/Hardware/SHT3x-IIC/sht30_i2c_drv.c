...
static uint8_t	SHT30_Send_Cmd(SHT30_CMD cmd)
{
    ...
    return i2c_transmit(SHT30_ADDR_WRITE, (uint8_t*)cmd_buffer, 2, 20);
}
...
{
    if (i2c_CheckDevice(SHT30_ADDR_WRITE) == 0) {
        printf("sht3x exist!\n");
    } else {
        printf("sht3x is not exist!\n");
    }
...
uint8_t SHT30_Read_Dat(uint8_t* dat)
{
	...
	//return HAL_I2C_Master_Receive(&hi2c2, SHT30_ADDR_READ, dat, 6, 0xFFFF);
    return i2c_receive(SHT30_ADDR_WRITE, dat, 6, 100);
}
