...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagtemp_max)
    {
        info->event[info->num] = EVENT_temp_max;
        info->num++;
        dataPoints->valuetemp_max = gizX2YFloat(temp_max_RATIO,  temp_max_ADDITION, (float)(exchangeBytes(issuedData->attrVals.valuetemp_max)));
    }
        
    if(0x01 == issuedData->attrFlags.flagtemp_min)
    {
        info->event[info->num] = EVENT_temp_min;
        info->num++;
        dataPoints->valuetemp_min = gizX2YFloat(temp_min_RATIO,  temp_min_ADDITION, (float)(exchangeBytes(issuedData->attrVals.valuetemp_min)));
    }
        
    if(0x01 == issuedData->attrFlags.flaghumi_max)
    {
        info->event[info->num] = EVENT_humi_max;
        info->num++;
        dataPoints->valuehumi_max = gizX2YFloat(humi_max_RATIO,  humi_max_ADDITION, (float)(exchangeBytes(issuedData->attrVals.valuehumi_max)));
    }
        
    if(0x01 == issuedData->attrFlags.flaghumi_min)
    {
        info->event[info->num] = EVENT_humi_min;
        info->num++;
        dataPoints->valuehumi_min = gizX2YFloat(humi_min_RATIO,  humi_min_ADDITION, (float)(exchangeBytes(issuedData->attrVals.valuehumi_min)));
    }
        
    if(0x01 == issuedData->attrFlags.flaggas_max)
    {
        info->event[info->num] = EVENT_gas_max;
        info->num++;
        dataPoints->valuegas_max = gizX2Y(gas_max_RATIO,  gas_max_ADDITION, exchangeBytes(issuedData->attrVals.valuegas_max));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valuetemp_max != cur->valuetemp_max)
    {
        GIZWITS_LOG("valuetemp_max Changed\n");
        ret = 1;
    }
    if(last->valuetemp_min != cur->valuetemp_min)
    {
        GIZWITS_LOG("valuetemp_min Changed\n");
        ret = 1;
    }
    if(last->valuehumi_max != cur->valuehumi_max)
    {
        GIZWITS_LOG("valuehumi_max Changed\n");
        ret = 1;
    }
    if(last->valuehumi_min != cur->valuehumi_min)
    {
        GIZWITS_LOG("valuehumi_min Changed\n");
        ret = 1;
    }
    if(last->valuegas_max != cur->valuegas_max)
    {
        GIZWITS_LOG("valuegas_max Changed\n");
        ret = 1;
    }
    ...
    if(last->valueEnterNum != cur->valueEnterNum)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueEnterNum Changed\n");
            ret = 1;
        }
    }
    if(last->valueExitNum != cur->valueExitNum)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueExitNum Changed\n");
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    devStatusPtr->valuetemp_max = exchangeBytes(gizY2XFloat(temp_max_RATIO,  temp_max_ADDITION, dataPoints->valuetemp_max));     
    devStatusPtr->valuetemp_min = exchangeBytes(gizY2XFloat(temp_min_RATIO,  temp_min_ADDITION, dataPoints->valuetemp_min));     
    devStatusPtr->valuehumi_max = exchangeBytes(gizY2XFloat(humi_max_RATIO,  humi_max_ADDITION, dataPoints->valuehumi_max));     
    devStatusPtr->valuehumi_min = exchangeBytes(gizY2XFloat(humi_min_RATIO,  humi_min_ADDITION, dataPoints->valuehumi_min));     
    devStatusPtr->valuegas_max = exchangeBytes(gizY2X(gas_max_RATIO,  gas_max_ADDITION, dataPoints->valuegas_max)); 
    ...
    devStatusPtr->valueGas = exchangeBytes(gizY2XFloat(Gas_RATIO,  Gas_ADDITION, dataPoints->valueGas));     

    devStatusPtr->valueEnterNum = exchangeWord(gizY2X(EnterNum_RATIO,  EnterNum_ADDITION, dataPoints->valueEnterNum)); 
    devStatusPtr->valueExitNum = exchangeWord(gizY2X(ExitNum_RATIO,  ExitNum_ADDITION, dataPoints->valueExitNum)); 
...
static int32_t gizProtocolGetDeviceInfo(protocolHead_t * head)
{
    ...
    deviceInfo.DataLen = GIZ_DATA_LEN;
